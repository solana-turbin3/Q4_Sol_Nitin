...
pub struct Stake<'info> {
    ...
    #[account(
        seeds = [b"metadata", metadata_program.key().as_ref(), mint.key().as_ref()], // by default it is derived from our program_id
        seeds::program = metadata_program.key(), // now it use metadata_program
        bump,
